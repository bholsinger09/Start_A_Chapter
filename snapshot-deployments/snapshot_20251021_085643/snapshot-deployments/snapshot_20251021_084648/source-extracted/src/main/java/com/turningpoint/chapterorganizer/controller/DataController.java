...
public class DataController {
    ...
    @Autowired
    public DataController(ChapterRepository chapterRepository,
    ...
    @PostMapping("/populate-all-states")
    public ResponseEntity<String> populateAllStatesData() {
        try {
            ...
            // Create chapters for all 50 states
            Chapter[] chapters = createAllStateChapters();
            
            // Create members and events for each chapter
            createMembersAndEventsForAllChapters(chapters);
            ...
            return ResponseEntity.ok(String.format(
                "Successfully populated data for all 50 states! Created %d chapters, %d members, and %d events.",
                chapterCount, memberCount, eventCount));

        } catch (Exception e) {
            return ResponseEntity.internalServerError()
                .body("Error populating data: " + e.getMessage());
        }
    }

    @GetMapping("/data-stats")
    public ResponseEntity<String> getDataStats() {
        long chapterCount = chapterRepository.count();
        long memberCount = memberRepository.count();
        long eventCount = eventRepository.count();
        
        List<String> states = chapterRepository.findAll().stream()
            .map(Chapter::getState)
            .distinct()
            .sorted()
            .toList();

        return ResponseEntity.ok(String.format(
            "Current data: %d chapters, %d members, %d events across %d states: %s",
            chapterCount, memberCount, eventCount, states.size(), String.join(", ", states)));
    }
    ...
    private void createMembersAndEventsForAllChapters(Chapter[] chapters) {
        String[] firstNames = {
        ...
        EventType[] eventTypes = {EventType.MEETING, EventType.SOCIAL, EventType.FUNDRAISER, EventType.VOLUNTEER, EventType.EDUCATIONAL, EventType.RECRUITMENT};
        
        Random random = new Random();
        
        for (Chapter chapter : chapters) {
            // Create 2-4 members per chapter
            ...
            for (int i = 0; i < memberCount; i++) {
                ...
                member.setEmail(member.getFirstName().toLowerCase() + "." + 
                               member.getLastName().toLowerCase() + "@" + 
                               getEmailDomain(chapter.getUniversityName()));
                member.setRole(i == 0 ? MemberRole.PRESIDENT : 
                              (i == 1 ? MemberRole.VICE_PRESIDENT : MemberRole.MEMBER));
                member.setActive(true);
                member.setChapter(chapter);
                memberRepository.save(member);
            }
            
            // Create 1-2 events per chapter
            int eventCount = 1 + random.nextInt(2);
            for (int i = 0; i < eventCount; i++) {
                Event event = new Event();
                event.setTitle(getEventTitle(eventTypes[random.nextInt(eventTypes.length)]));
                event.setDescription("Chapter event at " + chapter.getUniversityName());
                event.setType(eventTypes[random.nextInt(eventTypes.length)]);
                event.setEventDateTime(LocalDateTime.now().plusDays(random.nextInt(30)));
                event.setLocation(chapter.getUniversityName() + " Campus");
    ...
    private String getEmailDomain(String universityName) {
        if (universityName.contains("Alabama")) return "ua.edu";
        if (universityName.contains("Alaska")) return "alaska.edu";
        if (universityName.contains("Arizona")) return "asu.edu";
        ...
        if (universityName.contains("Louisiana")) return "lsu.edu";
        ...
        if (universityName.contains("Ohio")) return "osu.edu";
    ...
    private String getEventTitle(EventType type) {
        return switch (type) {
            case MEETING -> "Chapter Meeting";
            case SOCIAL -> "Social Mixer";
            case FUNDRAISER -> "Fundraising Event";
            case VOLUNTEER -> "Community Service";
            case EDUCATIONAL -> "Educational Workshop";
            case RECRUITMENT -> "New Member Recruitment";
